...
What happened? As you are not a data manager (so not a part of spider_data group) but a user (part of spider_user group), you do not have write permissions in the project's Data directory. However, all users have read permissions. This means data is managed by a manager and can be shared within the project without worrying about someone accidentally deleting/overwriting the data for the project.
